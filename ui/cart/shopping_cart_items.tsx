...
function CartItem({ cartItem }: { cartItem: ShoppingCartItem }) {
  ...
  const handleIncrease = async () => {
    ...
    try {
      await increaseQuantity(cartItem.cart_item_id); // Server call to update quantity
    } catch (error) {
      alert("Failed to update quantity. Please try again.");
      setQuantity((prevQuantity) => prevQuantity - 1); // Revert UI change on error
    }
  };
  ...
  const handleDecrease = async () => {
    setQuantity((prevQuantity) => prevQuantity - 1);
    try {
      await decreaseQuantity(cartItem.cart_item_id); // Server call to update quantity
    } catch (error) {
      alert("Failed to update quantity. Please try again.");
      setQuantity((prevQuantity) => prevQuantity + 1); // Revert UI change on error
    }
  ...
  return (
    <>
      <div className="flex flex-col items-center xsm:flex-row xsm:items-start gap-4">
        {imageLoading && <Skeleton width={170} height={170} />} 
        <img
          src={cartItem.productimages[0]}
          alt="product image"
          className={imageLoading ? 'hidden': 'max-w-[170px]'} 
      ...
      <div className="flex justify-center xsm:flex-col xsm:justify-between xsm:items-end">
        <RiDeleteBin6Fill
          className="text-red-600 text-lg cursor-pointer absolute top-2 right-2 xsm:relative xsm:top-0 xsm:right-0"
          onClick={() => deleteCartItem(cartItem.cart_item_id)}
        />
    ...
    <div className="flex flex-col md:flex-row gap-10 mb-4">
            ...
            <div
              ...
              className={`${
                ...
                index != shoppingCartItems.length - 1
                  ? "border-b-[1px] pb-6"
                  : "mb-2"
              } relative mb-6 border-gray-300 flex flex-col gap-2 xsm:flex-row xsm:justify-between`}
            >
      ...
      <div className="self-start w-full p-3 border-slate-300 border-[1px] rounded-xl">
        ...
        <div className="flex justify-between text-base my-4 py-2 border-t-[1px] border-gray-300 font-bold">
          ...
          <p className="font-bold">{`$${(totalCost + 0.08 * totalCost).toFixed(
            2
          )}`}</p>
        </div>
